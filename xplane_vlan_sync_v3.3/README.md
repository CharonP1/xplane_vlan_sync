# X-Plane TSN vs 标准以太网 混合同步演示

## 项目简介
本项目基于改造后的 XPC (XPlane Connect) 协议，实现了一台主机（Sender）控制多台从机（Receiver）的飞行姿态同步。
本系统演示了时间敏感网络 (TSN) 与 普通以太网 (Best Effort) 在同一网络环境下的性能差异。
主机使用单一物理网卡，采用混合发送模式：
1.  向 TSN 从机：构造带有 IEEE 802.1Q VLAN 标签 (VLAN 100) 和高优先级 (PCP 5) 的数据帧，直接通过 Raw Socket 发送。
2.  向 普通 从机：使用标准 UDP Socket 发送普通以太网数据帧 (Untagged)，模拟传统网络通信。

## 快速开始指南

### 阶段 1：环境准备 (所有机器)
在主机和两台从机上均需执行以下命令安装依赖并编译：
```bash
make install-deps
make build
```
...
- IP: `192.168.2.10`
- 链路特征: 经过普通以太网交换机，接收 普通以太网 数据帧，无 QoS 保障。
- 连接: 位于普通交换机 A 下游。
